...
def _create_train_data(datasets_name):
    ...
    datasets = [_read_csv(name).iloc[:600] for name in tqdm(datasets_name)]
...
class IsTrollClassifierModel:
    ...
    def __init__(self, embedder="bert", train_dataset="troll_data_big.csv"):
        """Init class object.
    ...
    def _load_or_create_embeddings(
            self, text, path, name, is_load=True, save=True):
    ...
    def fit(self):
        ...
        self._load_of_fit_features_extractor(
            texts, targets, is_load=False, is_save=False
        )
        #########################################################
        print("Read main troll dataset.")
        troll = read_csv(
        ...
        np_save("troll_features.npy", features)
        metrics = self._fit_troll_classiier(features, targets)
